function openerp_pos_widgets(instance, module){ //module is instance.point_of_sale
    ...
    module.ProductListWidget = module.PosBaseWidget.extend({
        ...
        render_product: function(product){
            ...
            var pos_categ_id = undefined;
            pos_categ_id =product.pos_categ_id;
            if(typeof(pos_categ_id) != undefined && typeof(pos_categ_id[1]) != undefined && (pos_categ_id instanceof Array) &&  pos_categ_id.length >1 ){
                try{
                    pos_cat_data_arr = pos_categ_id[1].split("/ ");
                }
                catch(err){
                    console.log(product.pos_categ_id);
                    console.error(err);
                }
                pos_cat_data_len = pos_cat_data_arr.length;
                product.pos_categ_id =pos_cat_data_arr[pos_cat_data_len-1];
            }
            if(!cached){
