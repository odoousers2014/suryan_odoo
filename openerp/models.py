...
class c_model(BaseModel):
    """Will override some methods here so that I can customize as per my wish."""
    _auto = True
    _register = False # not visible in ORM registry, meant to be python-inherited only
    _transient = False # True in a TransientModel
    
    def search(self, cr, user, args, offset=0, limit=None, order=None, context=None, count=False):
        args.append(['is_deleted','=', False]) # Explicit added condition for not showing deleted records
        return super(c_model, self).search(cr, user, args, offset, limit, order, context, count)

    def unlink(self, cr, uid, ids, context=None):
        """This will update data in case delete is trigger"""
        return super(c_model, self).write(cr, uid, ids, {'is_deleted': True} )
